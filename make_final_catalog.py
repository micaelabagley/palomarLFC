...
def run_SE(images, zp, wispfield, mode='dual'):
    ...
    if mode == 'dual':
        ...
        cat_g = base_g + '_final_cat.fits'
        cat_i = base_i + '_final_cat.fits'
        seg_i = base_i + '_final_seg.fits'
    ...
    if mode == 'single':
        for image in images:
            ...
            cat = base + '_final_cat.fits'
            seg = base + '_final_seg.fits'
...
def make_cat(Palcats, WISPfilters, threshold, wispfield):
###START HERE
    '''Calibrate Palomar photometry by:
        2) Match catalog to SDSS
        3) Calculate 0th order zeropoint (no color term)
        4) Apply zeropoint correction to Palomar g and i photometry
...
#    AUTO_i = AUTO_i + zp_i
    ...
    # consider only sources within 2sigma of median zero point
    g_lowsig,g_upsig = np.percentile(diff_g, 47.7),np.percentile(diff_g, 95.4)
    i_lowsig,i_upsig = np.percentile(diff_i, 47.7),np.percentile(diff_i, 95.4)
    ...
    # print to file
    with open('sdss_calibration.dat', 'w') as calfile:
        catfile.write('# Calibration information for %s\n'%wispfield)
        catfile.write('# \n')
        catfile.write('# %i sources used in fit'%sdss_g.shape[0])
...
def main():
    ...
    Palomar_cats = glob(os.path.join(wispfield,'%s_final_cat.fits'%wispfield))
    Palomar_cats.sort()
    ...
    make_cat(Palomar_catalogs, WISPfilters, threshold, wispfield)
